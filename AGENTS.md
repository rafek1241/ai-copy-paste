...
- **Starting the Driver**: The driver must be started on port 4445. Use `npm run driver:bg` for background mode or `npm run driver:debug` for visible mode.
- **Requirements**: WebView2 must be installed on the system.

To run tests manually:
- `npm run e2e-session` (default local mode, background window)
- `npm run e2e-session:debug` (visible window for debugging)
